...
LTBuilder& LTBuilder::SetExternalField(const ExternalField& phi_a_)
{
    phi_a = phi_a_;
    return *this;
}
...
LTBuilder& LTBuilder::SetEdgeInTheMiddle(double q)
 {
    ChargePtr first = std::make_shared<Charge>(Vector{(x_max + x_min) / 2, (y_max + y_min) / 2, (z_max + z_min) / 2}, q, 0);
    ChargePtr second = std::make_shared<Charge>(Vector{(x_max + x_min) / 2, (y_max + y_min) / 2, (z_max + z_min) / 2 + h}, q, 0);
